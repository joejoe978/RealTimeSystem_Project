...
static struct task_struct *pick_next_task_simple_rr(struct rq *rq)
{
	...
	// new
	struct list_head *lh;
	struct task_struct *tts;
	next = NULL;
	unsigned int min = 4294967295;
	
	list_for_each(lh, (&rq->simple_rr.queue)) {
		tts = list_entry(lh, struct task_struct, simple_rr_list_item);
		if (tts) {
			if (!(tts->haspicked)) {
				if ((tts->weighted_time_slice) < min) {
					min = tts->weighted_time_slice;
					next = tts;
				}
			} // has picked
		} // not NULL
	...
	next->haspicked = 1;
	//tts = list_entry((&rq->simple_rr.queue)->next, struct task_struct, simple_rr_list_item);
